...
public class MapDbVelvet implements IVelvet {
  ...
  private Supplier<ISerializer> serializerSupplier;
		...
		private class SortedStoreRequest extends AIndexedRequest<K, K> {
			...
			void gotoLowElement(IQueryAnchor<K, K> anchor) {
				if (anchor == null) {
					...
					cursor = anchor.isIncluding() ? valueMap.ceilingEntry(anchor.getKey())
							: valueMap.higherEntry(anchor.getKey());
				}
			}
			...
			void gotoHighElement(IQueryAnchor<K, K> anchor) {
				if (anchor == null) {
					...
					cursor = anchor.isIncluding() ? valueMap.floorEntry(anchor.getKey())
							: valueMap.lowerEntry(anchor.getKey());
				}
			}
	...
	@Override
	public <K, V, M extends Comparable<? super M>> IKeyIndexLink<K, M> secondaryKeyIndex(Object key1, String edgekind, Function<V, M> nodeMetric, Class<M> mclazz, Class<K> keyClazz, IStore<K, V> childStore) {
	...
	private abstract class AIndexedRequest<K, M extends Comparable<? super M>> {
		...
		List<K> go(IRangeQuery<K, M> query) {
			...
			if (query.isAscending()) {
				...
				forwardWhile(
						() -> (isBelow(query.getHighAnchor(), true)
		...
		boolean isBelow(IQueryAnchor<K, M> anchor, boolean below) {
			if (anchor == null)
				...
				if (compare == 0 && anchor.isIncluding())
					return true;
				return compare < 0 && below || compare > 0 && !below; // TODO
																		// XOR
			}
		}
	...
	private abstract class AMultiLink<K> implements ILink<K> {
		...
		abstract class ALinkRequest<M extends Comparable<? super M>> extends AIndexedRequest<K, M> {
			...
			void gotoLowElement(IQueryAnchor<K, M> anchor) {
				if (anchor == null) {
					cursor = connectSet.first();
				} else {
					Object[] anchorEl = anchorElement(anchor.getMetric());
					cursor = anchor.isIncluding() ? connectSet.ceiling(anchorEl) : connectSet.higher(anchorEl);
				}
			}
			...
			void gotoHighElement(IQueryAnchor<K, M> anchor) {
				if (anchor == null) {
					...
					Object[] anchorEl = anchorElement(anchor.getMetric());
					cursor = anchor.isIncluding() ? connectSet.floor(anchorEl) : connectSet.lower(anchorEl);
				}
			}
	 ...
	 */
	private class SecMultiLink<K, V, M extends Comparable<? super M>> extends AMultiLink<K>implements IKeyIndexLink<K, M> {
