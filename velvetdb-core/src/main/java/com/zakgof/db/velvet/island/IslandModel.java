...
public class IslandModel {
	...
	public static class Builder {
		...
		public class FetcherEntityBuilder<K, V> {
			...
			public <L, C extends Comparable<C>> FetcherEntityBuilder<K, V> include(IMultiLinkDef<K, V, ?, L> linkDef,
					Function<L, C> sortBy) {
				multis.put(linkDef.getKind(), new MultiConnector<>(linkDef));
				sorts.put(linkDef.getChildEntity().getValueClass(),
						Comparator.<DataWrap<L>, C>comparing(w -> sortBy.apply(w.getNode())));
				return this;
			}
			...
			public <CK, CV, M extends Comparable<? super M>> FetcherEntityBuilder<K, V> include(
					ISortedMultiLink<K, V, CK, CV, M> indexed, IRangeQuery<CK, M> query) {
			...
			//
			// public <L> FetcherEntityBuilder<K, V> withCollection(String name,
			// IContextMultiGetter<L> contextMultiGetter) {
			...
			public Builder done() {
				Builder.this.addEntity(new FetcherEntity<K, V>(entityDef, multis, singles, multiContexts,
						singleContexts, detaches, sorts));
				return Builder.this;
			}
	...
	private static class FetcherEntity<K, V> {
		...
		private FetcherEntity(IEntityDef<K, V> entityDef, Map<String, IMultiConnector<K, V, ?, ?>> multis,
				Map<String, ISingleConnector<K, V, ?, ?>> singles, Map<String, IContextMultiGetter<?>> multiContexts,
				Map<String, IContextSingleGetter<?>> singleContexts, List<? extends IBiLinkDef<K, V, ?, ?, ?>> detaches,
				Map<Class<?>, Comparator<?>> sorts) {
			this.entityDef = entityDef;
			this.multis = multis;
	...
	public <T> List<DataWrap<T>> fetchAll(IVelvet velvet, IEntityDef<?, T> entityDef) {
		List<DataWrap<T>> wrap = entityDef.get(velvet).stream()
				.map(node -> this.<T>createWrap(velvet, entityDef.getKind(), node, new Context()))
	...
	private <T> DataWrap<T> createWrap(IVelvet velvet, String kind, T node, Context context) {
		...
		if (entity != null) {
			...
			for (Entry<String, IContextMultiGetter<?>> entry : entity.multiContexts.entrySet()) {
				...
				List<DataWrap<?>> wrappedLinks = stream.map(o -> createWrap(velvet, kindOf(o), o, context))
						.collect(Collectors.toList());
				wrapBuilder.addList(entry.getKey(), wrappedLinks);
			}
	...
	private <T> DataWrap<?> wrapChild(IVelvet velvet, Context context, T node,
			ISingleConnector<?, T, ?, ?> singleConn) {
	...
	private <T, CK, CV> List<DataWrap<?>> wrapChildren(IVelvet velvet, Context context, FetcherEntity<?, T> entity,
			T node, IMultiConnector<?, T, CK, CV> multiConn) {
		Stream<DataWrap<CV>> stream = multiConn.getter().multi(velvet, node).stream().filter(l -> l != null) // TODO
																												// :
																												// check
																												// for
																												// error
	...
	// Natural sorting
	@SuppressWarnings({ "unchecked" })
	private <T, V> Stream<DataWrap<V>> decorateBySort(Stream<DataWrap<V>> stream, FetcherEntity<?, T> entity,
			Class<?> childClass) {
	...
	public static <K, V> List<DataWrap<V>> rawRetchAll(IVelvet velvet, IEntityDef<K, V> entityDef) {
		List<DataWrap<V>> nodes = entityDef.get(velvet).stream()
				.map(node -> new DataWrap<V>(node, entityDef.keyOf(node))).collect(Collectors.toList());
		return nodes;
	}
	...
	public <K, V> List<DataWrap<V>> getByKeys(IVelvet velvet, Collection<K> keys, IEntityDef<K, V> entityDef) {
		return decorateBySort(keys.stream().map(key -> getByKey(velvet, key, entityDef)),
				entities.get(entityDef.getKind()), entityDef.getValueClass()).collect(Collectors.toList());
	}

	/*
	 * public <T> void save(IVelvet velvet, Collection<DataWrap<T>> data) { for
	 * (DataWrap<T> wrap : data) save(velvet, wrap); }
	 * 
	 * public <T> void save(IVelvet velvet, DataWrap<T> data) { T node =
	 * data.getNode(); IEntityDef<?, T> entityDef = entityOf(node);
	 * entityDef.put(velvet, node); String kind = entityDef.getKind();
	 * 
	 * @SuppressWarnings("unchecked") FetcherEntity<?, T> entity =
	 * (FetcherEntity<?, T>) entities.get(kind); if (entity != null) { for
	 * (ISingleLinkDef<?, T, ?, ?> single : entity.singles.values())
	 * saveChild(velvet, data, single); for (IMultiLinkDef<?, T, ?, ?> multi :
	 * entity.multis.values()) saveChildren(velvet, data, multi); } }
	 * 
	 * private <T, B> void saveChild(IVelvet velvet, DataWrap<T> parentWrap,
	 * ISingleLinkDef<?, T, ?, B> singleLink) { DataWrap<B> childWrap =
	 * parentWrap.singleLink(singleLink); singleLink.connect(velvet,
	 * parentWrap.getNode(), childWrap.getNode()); save(velvet, childWrap); }
	 * 
	 * private <T, B> void saveChildren(IVelvet velvet, DataWrap<T> parentWrap,
	 * IMultiLinkDef<?, T, ?, B> multiLink) { List<DataWrap<B>> childrenWraps =
	 * parentWrap.multiLink(multiLink); for (DataWrap<B> childWrap :
	 * childrenWraps) { multiLink.connect(velvet, parentWrap.getNode(),
	 * childWrap.getNode()); save(velvet, childWrap); } }
	 */
