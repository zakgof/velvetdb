...
public class SecondarySortedLinkTest extends AVelvetTxnTest {
    ...
    public void testAllKeys() {
        List<String> result = Stream.of(MULTI.indexed(SecQueries.<Integer, Long>builder().build()).get(velvet, root)).map(TestEnt3::getStr).collect(Collectors.toList());
    ...
    private void check(ISecQuery<Integer, Long> query, Object... ref) {
        List<String> result = Stream.of(MULTI.indexed(query).get(velvet, root)).map(TestEnt3::getStr).collect(Collectors.toList());
