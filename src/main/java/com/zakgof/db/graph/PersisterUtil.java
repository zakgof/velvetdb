...
import com.zakgof.db.graph.annotation.Key;
import com.zakgof.db.graph.annotation.Kind;
...
public class PersisterUtil {
		...
		try {
			for (Field field : getAllFields(clazz)) {
				...
				if (field.getAnnotation(Key.class) != null
						|| field.getAnnotation(AutoKey.class) != null)
  ...
  public static <T> List<T> getAll(IPersister persister, List<Object> keys, Class<T> clazz) {
  ...
  public static <A,B> void upgrade(IPersister persister, Class<A> oldClass, Class<B> newClass, IFunction<A, B> convertor) {
    String kind = kindOf(newClass);
    if (!kind.equals(kindOf(oldClass)))
      throw new RuntimeException("Upgrage impossible: kinds not match");
    List<A> oldValues = persister.allOf(oldClass);
